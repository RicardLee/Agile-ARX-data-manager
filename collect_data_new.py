...
class RosOperator:
    ...
    def process(self):
        ...
        while (count < self.args.max_timesteps + 1) and not rospy.is_shutdown():
            ...
            #     key_input = sys.stdin.readline().strip().lower()
            #     print(f"检测到按键: {key_input}")
            #     break
            ...
            if count > 1:  # 有前一帧才比较
                ...
                if not (np.abs(delta_qpos) > 0.001).any() and not (np.abs(base_vel) > 0.001).any():
                    continue
            prev_qpos = qpos
            count += 1
            # 2.1 图像信息
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if key_input == 'q':
            ...
            # save_data(args, timesteps.copy(), actions.copy(), dataset_path)
        ...
        # user_choice = input("是否开始下一次采集？(y/n): ").strip().lower()
        # if user_choice != 'y':
        #     print("采集终止。")
        #     break
